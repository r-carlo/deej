...
Build fails with go.opencensus.io connection refused Pi-hole (or similar) is blocking the domain. Whitelist it and rebuild.

webkit2gtk4.0-devel not found You're likely on Fedora 40+ inside the container. Recreate the container with --image fedora:39.

Arduino not detected / permission denied on serial port Confirm dialout group membership (Step 6) and that you've rebooted since adding it.
